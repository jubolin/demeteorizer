...
Demeteorizer.prototype.convert = function (options, callback) {
  var context = {
    ...
    packageJSON   : JSON.parse(fs.readFileSync(path.join(options.input,'/package.json')))
  };
...
Demeteorizer.prototype.getBundleCommand = function (context) {
  var cmd = process.env.JUBO_PATH + '/meteor/meteor';
...
Demeteorizer.prototype.createPackageJSON = function (context, callback) {
  ...
  if(fs.existsSync(context.options.package_json)) {
    packageJSON.alias = context.packageJSON.alias;
    packageJSON.version = context.packageJSON.version;
    packageJSON.icon = context.packageJSON.icon;
    packageJSON.description = context.packageJSON.description;
    if(context.packageJSON.apps)
      packageJSON.apps = context.packageJSON.apps;
  }
  ...
  cmd = 'cp -r ' + path.join(context.options.input,'/',context.packageJSON.icon) +
    path.join(' ',context.options.output);
